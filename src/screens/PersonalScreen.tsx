...
import { Sparkles, BookOpen, Plus, Search, Heart, Pin, Trash2, FolderPlus, MoreVertical, Edit2 } from 'lucide-react';
    ...
    <div className="mx-auto max-w-3xl space-y-6 px-4 pb-10 pt-6">
      <section className="rounded-3xl border border-border bg-card p-6 shadow-sm">
        <div className="flex justify-between items-start mb-2">
          <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-gold">{getLocalizedText({ en: 'Collection', bn: 'সংগ্রহ' })}</p>
          {selectedSectionId !== 'all' && (
            <div className="flex gap-2">
              <button onClick={() => onEditSection(currentSection)} className="p-1.5 text-text-muted hover:text-gold transition-colors"><Edit2 size={14} /></button>
              <button onClick={() => onDeleteSection(selectedSectionId)} className="p-1.5 text-text-muted hover:text-red-400 transition-colors"><Trash2 size={14} /></button>
            </div>
          )}
        </div>
        <h2 className="text-2xl font-bold text-text-main">{getLocalizedText(currentSection.name)}</h2>
        <p className="mt-2 text-sm leading-relaxed text-text-sub">{getLocalizedText({ en: 'Organize your spiritual journey with custom collections.', bn: 'আপনার ইবাদতের রুটিন কালেকশন দিয়ে সাজিয়ে নিন।' })}</p>
